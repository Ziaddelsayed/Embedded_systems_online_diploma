...
typedef struct {
	...
	uint8_t				    IRQ_EN;         //Enable or Disable the EXTI IRQ (That will Enable the IRQ mask in EXIT and also on NVIC interrupt controller)
											//This parameter can be a value of  @ref EXTI_IRQ_define.

	void(*P_IRQ_CallBack)(void);			//C Function that will be called once the IRQ happened.
